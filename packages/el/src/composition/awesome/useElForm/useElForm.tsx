...
import {computed, defineComponent, PropType, ref, watch, Component, unref, getCurrentInstance, KeepAlive} from "vue";
import {ElForm, ElFormItem, ElInput, ElSelect, ElOption} from "element-plus";
import {IUFItem} from "@/composition/useElForm/useElFormHelper";
 ...
 */
export const useElFormConf = <IModel=any>(itemList: Ref<FormRuleItem<IModel>[]> | FormRuleItem[]) => {
     ...
     */
    const formValidateTo = () => awaitTo(formRef.value?.validate() ?? Promise.reject('表单实例未初始化'));
    const formValidateFiledTo = (field: keyof IModel) => awaitTo(formRef.value?.validateField(field as string) ?? Promise.reject('表单实例未初始化'));
...
const generateUFItem = <IModel = any>(model: Ref<IModel>, formItemList: Ref<FormRuleItem[]>): IUFItem => {

    const UFItem = defineComponent({

        props: {
            prop: {
                type: [String, Number] as PropType<keyof IModel>,
                required: false,
            },
        ...
        setup(props, {slots, emit}) {
            ...
            return () => {
                ...
                if (!prop) {
                    return <ElFormItem {...formItemBind.value}><DefaultSlot /></ElFormItem>
                }
